defmodule SinghSabhaWeb.UserAuthTest do
  use SinghSabhaWeb.ConnCase, async: true
  ...
  import SinghSabha.AccountsFixtures

  @remember_me_cookie "_singh_sabha_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, SinghSabhaWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      SinghSabhaWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        SinghSabhaWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
